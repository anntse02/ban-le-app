...
export default function SalesForm({ onAddRecord, loading = false }: SalesFormProps) {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xs border border-slate-200 p-3 sm:p-4 relative overflow-hidden text-slate-800">
      ...
      <form onSubmit={handleSubmit} className="space-y-2.5 sm:space-y-3">
        ...
        <div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 p-2 sm:p-2.5 rounded-2xl border-2 text-left transition-all active:scale-[0.98] ${
                seller === "Hằng"
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  <span className={`font-black text-xs sm:text-sm ${seller === "Hằng" ? "text-pink-900" : "text-slate-700"}`}>
                    Hằng
                  </span>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  <span className={`font-black text-xs sm:text-sm ${seller === "Gấm" ? "text-purple-900" : "text-slate-700"}`}>
                    Gấm
                  </span>
        ...
        <div>
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <button
              ...
              className={`h-10 px-2 rounded-xl text-xs font-black border transition-all flex items-center justify-center gap-1 active:scale-95 ${
                isRetail
            ...
            <button
              ...
              className={`h-10 px-2 rounded-xl text-xs font-bold border transition-all flex items-center justify-between gap-1 active:scale-95 ${
            ...
            <input
              ...
              className={`h-10 px-2.5 bg-slate-50 border rounded-xl text-xs font-medium text-slate-900 focus:bg-white focus:ring-1 focus:ring-green-500 outline-none transition ${
        ...
        <div>
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="h-10 px-2.5 rounded-xl text-xs font-bold text-green-700 hover:text-green-800 flex items-center gap-1 bg-green-50 hover:bg-green-100 border border-green-200 transition shrink-0"
          ...
          {/* BẢNG QUẢN LÝ DANH MỤC HÀNG HÓA & CẤU HÌNH LOẠI BAO */}
            ...
            <div className="mt-2 p-2.5 bg-slate-50 rounded-xl border border-green-500/40 shadow-xs space-y-2 animate-fadeIn">
              <div className="flex items-center justify-between pb-1 border-b border-slate-200">
                <span className="text-[11px] font-bold text-slate-800">
                  ✏️ Sửa Tên, Loại Bao & Giá ({products.length} hàng):
                </span>
                <button
                  type="button"
              ...
              <div className="max-h-56 overflow-y-auto space-y-1.5 pr-0.5">
                {products.map((p, index) => (
                  <div
                    key={p.id}
                    className="flex items-center gap-1.5 p-1.5 bg-white rounded-xl border border-slate-200 text-xs"
                  >
                    {/* STT */}
                    <span className="text-[10px] font-bold text-slate-400 w-3.5 text-center shrink-0">
                      {index + 1}
                    </span>
                    ...
                    <input
                      ...
                      className="flex-1 min-w-[70px] px-2 py-1 text-xs font-semibold text-slate-800 border border-slate-200 rounded-lg outline-none focus:border-green-500"
                    />

                    {/* Checkbox 25kg */}
                    <label className="flex items-center gap-0.5 cursor-pointer select-none shrink-0" title="Cho phép bao 25kg">
                      <input
                        type="checkbox"
                    ...
                    {/* Checkbox 50kg */}
                    <label className="flex items-center gap-0.5 cursor-pointer select-none shrink-0" title="Cho phép bao 50kg">
                      <input
                        type="checkbox"
                    ...
                    {/* Ô nhập giá có dấu chấm */}
                    <div className="relative w-22 sm:w-26 shrink-0">
                      <input
                        type="text"
                        inputMode="numeric"
                        ...
                        placeholder="Giá..."
                        className="w-full px-1.5 py-1 pr-3.5 text-xs font-bold text-right text-slate-900 border border-slate-200 rounded-lg outline-none focus:border-green-500"
                      />
                    ...
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition shrink-0"
                      title="Xóa hàng này"
                    >
              ...
              <div className="flex items-center gap-1.5 pt-1 border-t border-slate-200">
                <button
                  ...
                  className="flex-1 py-1.5 px-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[11px] font-bold flex items-center justify-center gap-1 transition active:scale-95"
                >
                ...
                <button
                  type="button"
                  onClick={() => setIsProductManagerOpen(false)}
                  className="py-1.5 px-3 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl text-[11px] font-bold transition active:scale-95"
                >
                  Xong
                </button>
              </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className={`py-2.5 px-2 rounded-2xl border-2 font-bold text-xs sm:text-sm flex items-center justify-center gap-1.5 transition-all ${
              !isAllow25kg
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <div className="flex items-stretch h-11 rounded-2xl overflow-hidden border border-slate-300">
              <button
                type="button"
                onClick={() => setQuantity((prev) => Math.max(1, (Number(prev) || 1) - 1))}
                className="w-11 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-800 font-black text-xl flex items-center justify-center select-none transition active:scale-95 border-r border-slate-300 shrink-0"
              >
                −
              </button>
              <input
                ...
                className="w-full bg-slate-50 text-base font-black text-center text-slate-900 focus:bg-white focus:outline-none transition"
                required
              />
              <button
                type="button"
                onClick={() => setQuantity((prev) => (Number(prev) || 0) + 1)}
                className="w-11 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-800 font-black text-xl flex items-center justify-center select-none transition active:scale-95 border-l border-slate-300 shrink-0"
              >
                +
              </button>
            </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-11 px-3 bg-slate-50 border border-slate-300 rounded-2xl text-sm sm:text-base font-black text-slate-900 focus:bg-white focus:ring-1 focus:ring-green-500 outline-none transition"
              required
            />
          </div>
        </div>
        ...
        <div className="bg-slate-50 p-2 sm:p-2.5 rounded-2xl border border-slate-200 space-y-1.5">
          <label className="flex items-center justify-between cursor-pointer select-none">
            <div className="flex items-center gap-1.5">
              <Layers className="w-4 h-4 text-teal-600" />
              <span className="text-xs sm:text-sm font-bold text-slate-800">Khách lấy nhiều lần (Gửi kho)</span>
            </div>
            <input
              type="checkbox"
              ...
              className="w-4.5 h-4.5 rounded text-green-600 focus:ring-green-500"
            />
          </label>
          ...
          {isPartialPickup && (
            <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-200 text-xs animate-fadeIn">
              <span className="text-slate-600 font-semibold">Lần này lấy trước:</span>
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="w-16 h-8 px-2 text-center text-xs font-black bg-white border border-slate-300 rounded-xl outline-none focus:border-green-500"
                />
        ...
        <div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className={`h-10 sm:h-11 rounded-2xl font-black text-xs sm:text-sm flex items-center justify-center gap-1.5 border-2 transition-all active:scale-[0.98] ${
                paymentStatus === "paid"
            ...
            >
              <span>✓</span>
              <span>Đã thu</span>
              {paymentStatus === "paid" && <CheckCircle2 className="w-4 h-4 text-white" />}
            </button>
            ...
            >
              <span>✗</span>
              <span>Chưa thu</span>
              {paymentStatus === "unpaid" && <CheckCircle2 className="w-4 h-4 text-white" />}
            </button>
          </div>
        </div>

        {/* 8. KHUNG THÀNH TIỀN TẠM TÍNH */}
        <div className="bg-gradient-to-r from-emerald-50 to-green-50 border border-green-200 py-2 px-3.5 rounded-2xl flex items-center justify-between shadow-2xs">
          ...
          {numQty > 0 && numPrice > 0 && (
            <span className="text-[10px] sm:text-[11px] font-bold text-green-800/80 bg-white/80 px-2 py-0.5 rounded-lg border border-green-200">
        ...
        <div>
          <input
            ...
            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm focus:bg-white focus:ring-1 focus:ring-green-500 outline-none transition"
          />
        </div>
